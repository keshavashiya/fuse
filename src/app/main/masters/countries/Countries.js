...
const Countries = () => {
	...
	const { getSuccess, getLoading, deleteSuccess, deleteError } = Reducer.country;
	...
	const onDeleteClick = rowObj => {
		confirm({ description: 'Do you want to delete country?' })
			.then(() => {
		...
		() => [
			{
				...
				Header: ({ selectedFlatRows }) => {
					// eslint-disable-next-line react/prop-types
					const selectedRowIds = selectedFlatRows.map(row => row.original.idCountry);
