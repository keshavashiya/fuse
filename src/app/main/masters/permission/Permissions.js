...
import { Button, Typography, IconButton, Icon } from '@material-ui/core';
import {
...
import { PaginationTable, Search } from '../../../components';
...
const Permissions = () => {
	...
	const onDeleteClick = rowObj => {
		confirm({ description: 'Do you want to delete user role?' })
			.then(() => {
		...
		() => [
			{
				...
				Header: ({ selectedFlatRows }) => {
					// eslint-disable-next-line react/prop-types
					const selectedRowIds = selectedFlatRows.map(row => row.original.idPriceList);
					// eslint-disable-next-line react/prop-types
				...
				header={
					<div className="flex flex-1 w-full items-center justify-between">
						...
						<FuseAnimate animation="transition.slideRightIn" delay={300}>
							<Button
								component={Link}
								to={{ pathname: '/addpermission', edit: false, view: false }}
