...
const Areas = () => {
	...
	const onDeleteClick = rowObj => {
		confirm({ description: 'Do you want to delete area?' })
			.then(() => {
	...
	const childProps = obj => {
		if (obj.type === 'EDIT') {
			dispatch(
				showMessage({
					message: 'Area edit successfully',
		...
		() => [
			{
				...
				Header: ({ selectedFlatRows }) => {
					// eslint-disable-next-line react/prop-types
					const selectedRowIds = selectedFlatRows.map(row => row.original.idArea);
			...
			},
			{
				...
				Cell: ({ row }) => (
					<div className="flex items-center">
						{isPermission(authrole.Area, 'areas:edit') && (
							<IconButton
								size="small"
								onClick={ev => {
									ev.stopPropagation();
						...
						{isPermission(authrole.Area, 'areas:delete') && (
							<IconButton
								size="small"
								onClick={ev => {
									ev.stopPropagation();
	...
	return (
		<div>
			<FusePageCarded
				header={
					<div className="flex flex-1 w-full items-center justify-between">
							...
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<div className="flex items-center">
									<FuseAnimate animation="transition.expandIn" delay={300}>
										<Icon className="text-32">crop_portrait</Icon>
									</FuseAnimate>
						...
						{isPermission(authrole.Area, 'areas:add') && (
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Button
			...
			{openEditDrawer && (
				<CustomDrawer
					title={selectedData && selectedData.Name ? selectedData.Name : 'Area'}
					component={Area}
					onDrawerClose={closeDrawer}
					onCloseDrawer={closeDrawer}
